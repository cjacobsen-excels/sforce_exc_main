        ...
        helper
          ...
          .then((data) => {
            ...
            if (reqStatus !== 'Met') {
              ...
              for (let item of data.data.items) {
                if (item.value.attributes.type == "Faculty_Management__mdt") {
                  
                  subject = item.value.Name_Subject__c;
                  description = item.value.Description_Text_Area__c;
                } else if (item.value.attributes.type == "Group") {
                  ownerId = item.value.Id
                } else if (item.value.attributes.type == "CPNE_Qualification__c") {
                  whatId = item.value.Id;
                }
              }
              ...
              let newTask = {
                ...
                value: {
                  ...
                  OwnerId: ownerId,
                  WhatId: whatId,
                  WhoId: selectedUser.value.Id,
                  Subject: subject,
                  Description: description
                }
              }

              console.log("MAXIMUS", newTask);

              schedulesAndTasks.push(newTask);
            }

            helper
              .standardizeCallout(component, "upsertAll", {
                viewModel: JSON.stringify({
                  data: { items: schedulesAndTasks },
                  queries: []
                })
              })
              .then((data) => {
