({
  ...
  processData: function (component) {
    ...
    // build a list of qualified faculty contact ids, these are the only one we should use!
    let filteredContactIds = [];
    let queries = { queries: [] };
    queries.queries.push({
      fields: ["Id", "Faculty__c"],
      sObjectType: "Qualified_Faculty__c",
      whereClause: "Course__c = '" + vm.data.hed__course_offering__c[0].value.hed__Course__c + "' AND Active_Inactive__c = 'Active'",
      subQueries: [
        {
          fields: ["Id", "FirstName", "LastName"],
          sObjectType: "Contact",
          whereClause: "Id = '$PARENT.Faculty__c'"
        }
      ]
    });
    
    this.rawQuery(component, queries).then((data) => {
      for (let qualifiedFaculty of data.data.items) {
        filteredContactIds.push(qualifiedFaculty.value.Faculty__c);
      }

      let contactsQuery = { queries: [] };
      contactsQuery.queries.push({
        fields: ["Id", "FirstName", "LastName"],
        sObjectType: "Contact",
        orderBy: "LastName ASC",
        whereClause: "Id IN ('" + filteredContactIds.join("','") + "') AND Qualifications_Met__c=true",
        subQueries: [
          {
      ...
      this.rawQuery(component, contactsQuery).then((contactsData) => {
        let filteredContacts = [];
        let i = 0;
        for (let contact of contactsData.data.items) {
          if (filteredContactIds.includes(contact.value.Id)) {
        ...
        vm.data.contact = contactsData.data.items;
        component.set("v.viewModel", vm);
